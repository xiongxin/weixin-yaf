...
class Medoo {
    ...
    public function connect($retry = 3) {
        ...
        try {
            ...
            SeasLog::error('数据库查询失败SQL: ERR_MSG'.$e->getMessage());
            $this->connect( --$retry);
        }
    }
...
//            if($e->getCode() >= 2003 && $e->getCode() <= 2006){
            $this->connect();
//            }

            SeasLog::error('数据库查询失败SQL:'.$query.' ERR_MSG'.$e->getMessage());
...
//            if($e->getCode() >= 2003 && $e->getCode() <= 2006){
            $this->connect();
//            }
//            $this->connect();//这里好像有个坑，如果在外面开启事务，如果有一次sql执行失败，这个connect会将连接重置，导致在commit或rollback的时候报错：There is no active transaction
